        ...
        public async Task<WeatherAnalysisResult> AnalyzeImageAsync(Bitmap image, AstroContext? astroContext = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var payload = new
                {
                    model = _modelName,
                    temperature = 0.1,
                    // Room for a reasoning phase and the answer together: o-series models
                    // spend this budget thinking first, and 512 left nothing for the JSON
                    // (issue #16, seen on Gemini Flash for the same reason).
                    max_tokens = 2048,
                    messages = new object[]
                    {
                        new { role = "system", content = WeatherAnalysisPrompts.DetailedSystemPrompt },
                        new {
                            role = "user",
                            content = new object[]
                            {
            ...
            {
                Logger.Warning($"OpenAI API call timed out, falling back to local analysis: {ex.Message}");
                var fallback = new LocalWeatherAnalysisService();
            ...
            catch (WeatherResponseParseException ex)
            {
                ...
                Logger.Warning($"OpenAI answer could not be parsed, falling back to local analysis: {ex.Message}");
